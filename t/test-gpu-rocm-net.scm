...
(define (test-cpu-net-run net input)
  (let ((arrs (map (lambda (rv) (gpu-array rv)) (array->list (netr-arrs net)))))
    (match arrs
...
(define-test (test-gpu-rocm-net)
  (let* ((hlen 40)
         ...
         (cnet (randomize-net (make-net #:in 198 #:out 2 #:hid hlen)))
         (gnet (test-copy-net (make-net #:in 198 #:out 2 #:hid hlen) cnet)))
    (array-map! vxi (lambda (x) (* 0.01 (- (random-uniform) .5))) vxi)
    (test-cpu-net-run cnet vxi)
    (net-run gnet vxi)
    (array-for-each (lambda (rv) (gpu-refresh rv)) (netr-arrs gnet))
    (test-array (netr-arrs cnet) (netr-arrs gnet))))
