...
(define (lstm-backward da caches Wf bf Wi bi Wo bo Wc bc Wy by)
  (let* ((caches-last (car caches))
         (x (cadr caches)))
    (match (car caches-last)
      ((a1 c1 a0 c0 f1 i1 cc1 o1 x1)
       (match (list (array-dimensions da)
                    (array-dimensions x1))
         (((n_a m T_x) (n_x m))
          (let ((dx   (new-arr (list n_x m T_x)))
            ...
            (do ((t (1- T_x) (1- t))) ((< t 0))
              ...
              (arr-+! d da_prevt)
              (match (lstm-cell-backward d dc_prevt (list-ref caches-last t) Wf bf Wi bi Wo bo Wc bc Wy by)
            ...
            (set! da0 da_prevt)
            (list dx da0 dWf dbf dWi dbi dWc dbc dWo dbo))))))))
